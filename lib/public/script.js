...
$(function() {
  ...
  function renderNewQuote(quote) {
    ...
    $("blockquote").fadeOut(animationTimeout, function() {
      $(this).find(".content").text(quote.c).end()
             .find(".author").text(quote.a).end();
      showQuote(quote);
    });
  }
  ...
  function pushState(quote) {
    var path = "?id=" + quote.id;
    History.pushState(quote, null, path);
  }

  var animationTimeout = 1000;
  History.replaceState(CURRENT_QUOTE, null, "?id=" + CURRENT_QUOTE.id);
